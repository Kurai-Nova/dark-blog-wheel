...
На телефоне должна быть разрешена отладка по USB (у разных вендоров включается по разному), а на компьютере установлен USB-драйвер устройства (иногда само работает).
...
Хэлпер с примитивным гуем для PowerShell: [https://github.com/farag2/ADB-Debloating/tree/master](https://github.com/farag2/ADB-Debloating/tree/master)
...
## Реклама и телеметрия
  ...
  com.facebook.system, com.facebook.services (Facebook App Installer, Facebook App Manager, Facebook Services) – Мусорные сервисы Meta (запрещена в РФ). Также лицокнига может ставить службу с именем facebook.installer.
...
## Системные сервисы и утилиты
  ...
  Генератор журналов (com.bsp.catchlog) – Помогает составлять системные журналы, например, для просмотра данных о состоянии батареи смартфона. Редко используемое, к тому же есть более продвинутые аналоги.
...
## Google-сервисы
  ...
  Google One Time Init (com.google.android.onetimeinitializer) – Используется при первоначальной настройке смартфона, при сбросе до заводских настроек осстанавливается автоматически.
...
## ПРЕДУПРЕЖДЕНИЕ: отключение может повлечь за собой СУЩЕСТВЕННЫЕ ограничения в функционале смартфона
...
# ОПАСНО: ниже - системные приложения, есть РИСК ОКИРПИЧИТЬ СМАРТ
# ВНИМАНИЕ: ЗАМЕНИТЕ Клавиатуру и Сообщения ПЕРЕД удалением
...
# ВНИМАНИЕ: Получите бесконечные уведомления, ЕСЛИ НЕ ОТКЛЮЧИЛИ приложения выше!
...
Имя пакета можно также поискать, например тут (имя сервиса видно на странице загрузки APK): [https://apkpure.net/ru/my-honor/com.hihonor.phoneservice/download]

Меню сим-карт, согласно отзывам, можно удалить: _«На телефонах с процессором MTK удалял `Stk1.apk` из системной папки и меню SIM карт пропадало»_

Для МегаФон: можно запретить запуск «StkAppService».
...
Тема с обсуждением вопроса на 4PDA: https://4pda.to/forum/index.php?showtopic=236256
